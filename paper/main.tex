...
\begin{abstract}
We present a machine-learning approach to event reconstruction and gamma/hadron
separation for imaging atmospheric Cherenkov telescopes (IACTs), implemented as a
thin layer on top of established analysis pipelines.
For reconstruction, a single XGBoost model performs multi-target residual regression,
simultaneously correcting the event direction (two angular components) and energy
predicted by classical methods, using more than 100 image and stereo features.
...
Array Observatory (CTAO) Northern array.
Performance is evaluated on simulated events and on Crab Nebula observations,
demonstrating improvements in angular and energy resolution over the baseline.
\end{abstract}
...
The field knows a variety of reconstruction methods for direction and energy estimation,
...
(residuals) to the existing DispBDT reconstruction, effectively combining the robustness
...
Section~\ref{sec:classification} describes the gamma/hadron classification model.
...
validate the reconstruction and classification performance on observational data.
...
This paper presented a machine-learning approach to event reconstruction and
gamma/hadron separation for ground-based IACTs, demonstrated for VERITAS and
