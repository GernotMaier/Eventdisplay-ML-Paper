...
The analysis is based on a single multi-target regression approach with the following three targets:
logarithm of the primary energy ($\log_{10}(E)$) and the two-dimensional direction offsets
in camera coordinates ($x, y$).

Input features are derived from the standard Eventdisplay reconstruction pipeline
and include image parameters as width, length, size, and distance for each telescopes,
...
\subsubsection{Telescope-level features}
