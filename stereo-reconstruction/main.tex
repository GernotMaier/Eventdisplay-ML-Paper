...
corrects simultaneously reconstruction energy and direction results using
multi-target residual regression.
Features are beside the estimated energies and directions from several
reconstruction methods also image and stereo parameters
based mostly on classical methods like
...
Imaging Atmospheric Cherenkov Telescopes (IACTs) are designed to detect
photons with energies from tens of GeV to hundreds of TeV (e.g., \citep{2009ARA&A..47..523H}).
and to measure their energy and direction.
...
optimized to suppress the background light from the night sky.
...
The IACT community has developed a variety of reconstruction methods for direction and energy estimation.
This includes robust and efficient geometric methods based on image parameters
\citep{1985ICRC....3..445H, 1991ApJ...377L..97A, 1997APh.....8....1D, 1998APh.....8..179C, 1999APh....12..135H},
template-model based methods
\citep{2001A&A...374..895P,2006APh....25..195L, 2009APh....32..231D,2014APh....56...26P, 2024APh...16303008S},
and machine-learning-based methods using
Hillas parameters \citep{2024A&A...691A.328A} or full camera images
\citep{2019ICRC...36..752N}.
...
An additional step to improve performance is to select events
based on their estimated reconstruction uncertainty
\citep{2023arXiv230911375B, 2026arXiv260601059Z}, allowing to trade off event
statistics and reconstruction accuracy.
...
(residuals) to existing baseline reconstruction methods.
...
as demonstrated for both VERITAS and simulations of the
Northern and Southern arrays of the Cherenkov Telescope Array Observatory (CTAO)
...
Imaging atmospheric Cherenkov telescopes (IACTs) detect the Cherenkov light emitted
by extensive air showers as they propagate through the atmosphere.
The resulting camera image represents a projection of the longitudinal
development of the shower and encodes information about the energy and arrival direction of
the primary particle.
The reconstruction methods described below provide estimates of these quantities
with varying statistical precision and systematic biases, depending on the primary
particle energy, shower geometry, image quality, and observing conditions.
Their reconstructed quantities, together with intermediate reconstruction products,
image parameters, and array geometry, are used as input features for the
machine-learning regression model to improve the baseline event reconstruction.

We assume in the following familiarity of the reader with the basic analysis concepts for IACTs,
including calibration of each pixel signal, image cleaning, and image parameterisation
(see e.g., \cite{1997APh.....6..343A, 2006APh....25..391H}).


\subsection{Baseline direction reconstruction}

For direction reconstruction, results from a \textit{line-intersection} method and a
\textit{displacement method (dipsBDT)} are used as baseline.
The line-intersection method \citep{1991ApJ...377L..97A} determines
the shower direction in camera coordinates (features $X_{intersect}, Y_{intersect}$)
by calculating the weighted average of the intersection points of pairs of
image major axes.
Weighting is crucial to suppress the influence of poorly reconstructed images,
and typically a weighting scheme based on image size, shape, and angle between
the image pairs is used.

The displacement method (DispBDT) is a modified version of

\begin{itemize}
  \item MAGIC DISP \citep{2016APh....72...76A}
  \item Whipple \citep{1998APh.....8..179C}
\end{itemize}

\subsection{Baseline energy reconstruction}

\note{1-3 sentences on the general analysis pipelines steps;
that there are stereo events, telescope events, etc}
...
\note{Incorrect - especially energy resolution}
...
\citep{2015ICRC...34..792M} as a cross-check of the energy reconstruction.
...
\todo{Figure: Crab Nebula differential energy spectrum (XGBoost reconstruction)
overlaid with published VERITAS/MAGIC/HESS measurements \citep{2015ICRC...34..792M},
Quote flux normalisation and spectral index.}
